...
public class Rope : MonoBehaviour
{
    ...
    private void ApplyConstraint()
    {
        ...
        for (int i = 0; i < vertices - 1; i++)
        {
            ...
            float distance = Vector3.Distance(thisSeg.posNow, nextSeg.posNow);
            float error = distance - ropeSegLength;
            Vector3 changeDir = (thisSeg.posNow - nextSeg.posNow).normalized;
            Vector3 changeAmount = changeDir * error;

            if (i != 0)
            {
                // Dont change the position of the first link, because it is attached to the origin.
